...
public class MTCNN {
    ...
    public Rect getBiggestFace(Bitmap bitmap, int minFaceSize){
        Vector<Box> boxes=detectFaces(bitmap,minFaceSize);
        Box biggest=null;
        for (int i=0;i<boxes.size();i++){
            if (biggest==null ||(biggest.area()<boxes.get(i).area()))
                biggest=boxes.get(i);
        }
        if (biggest==null) return null;
        return biggest.transform2Rect();
    }
}
